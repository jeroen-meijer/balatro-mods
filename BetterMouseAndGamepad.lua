...
function love.mousereleased(x, y, button)
    if mouse_button_mapping[button] == "left_mouse_button" then
        C:L_cursor_release(x, y)
    end
    if mouse_button_mapping[button] == "right_mouse_button" then
        R_cursor_release(x, y)
    end
    if mouse_button_mapping[button] == "middle_mouse_button" then
        M_cursor_release(x, y)
    end
end
...
function Controller.update(self, dt)
    ...
    if R_dragging.can and not R_dragging.start and R_dragging.handled and R_clicked.handled and R_cursor_down.target then
    ...
    if not R_cursor_up.handled then
        ...
        if R_cursor_down.target and mod_functions_can["right_mouse_button_click"] then
            if (not R_cursor_down.target.click_timeout or R_cursor_down.target.click_timeout * G.SPEEDFACTOR >
                R_cursor_up.time - R_cursor_down.time) then
    ...
    if not M_cursor_up.handled then
        if not mod_functions_can["middle_mouse_button_click"] or ((C.locked) and not G.SETTINGS.paused) or
            (C.locks.frame) or (C.frame_buttonpress) then
        else
    ...
    if not R_clicked.handled then
        if not G.SETTINGS.paused and G.hand and G.hand.highlighted[1] then
            if (G.play and #G.play.cards > 0) or (C.locked) or (C.locks.frame) or
    ...
    if not R_dragging.handled and C.hovering.target and C.hovering.target:is(Card) and C.hovering.target.area == G.hand then
...
function Controller:button_press_update(button, dt)
    ...
    if C.button_registry[button] and C.button_registry[button][1] and
        not C.button_registry[button][1].node.under_overlay then
        ...
        if button == "a" then
            if C.focused.target and C.focused.target.config.focus_args and C.focused.target.config.focus_args.type ==
                'slider' and (not G.CONTROLLER.HID.mouse and not G.CONTROLLER.HID.axis_cursor) then
            else
                C:L_cursor_press()
            end
        end
        ...
        if button == 'b' then
            if G.hand and C.focused.target and C.focused.target.area == G.hand and mod_functions_can["b_click_and_hold"] then
...
function Controller:button_hold_update(button, dt)
    ...
    if ((C.locked) and not G.SETTINGS.paused) or (C.locks.frame) then
        return
    end
    -- modification of this function end
    C.frame_buttonpress = true
...
function queue_X1_cursor_press()
    if C.locks.frame or not mod_functions_can["x1_click"] then
        return
    end
...
function queue_X2_cursor_press()
    if C.locks.frame or not mod_functions_can["x2_click"] then
        return
    end
    if not G.SETTINGS.paused and G.STATE == G.STATES.SELECTING_HAND then
        G.FUNCS.sort_hand_value()
...
function queue_U_wheel_press()
    if C.locks.frame or not mod_functions_can["middle_mouse_button_up"] then
        return
    end
    if not G.SETTINGS.paused and G.STATE == G.STATES.SELECTING_HAND then
...
function queue_D_wheel_press()
    if C.locks.frame or not mod_functions_can["middle_mouse_button_down"] then
        return
    end
    if not G.SETTINGS.paused and G.STATE == G.STATES.SELECTING_HAND then
...
function R_cursor_press(x, y)
    ...
    if ((C.locked) and (not G.SETTINGS.paused or G.screenwipe)) or (C.locks.frame) then
        return
    end
