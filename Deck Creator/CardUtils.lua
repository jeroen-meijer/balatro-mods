...
local function applyRandomAbilities(card_protos, config)
    ...
    local abilities = {{
        name = 'polychrome',
        count = config.random_polychrome_cards,
        isEdition = true
    }, {
        name = 'holographic',
        count = config.random_holographic_cards,
        isEdition = true
    }, {
        name = 'foil',
        count = config.random_foil_cards,
        isEdition = true
    }, {
        name = 'm_bonus',
        count = config.random_bonus_cards,
        isEdition = false
    }, {
        name = 'm_mult',
        count = config.random_mult_cards,
        isEdition = false
    }, {
        name = 'm_wild',
        count = config.random_wild_cards,
        isEdition = false
    }, {
        name = 'm_glass',
        count = config.random_glass_cards,
        isEdition = false
    }, {
        name = 'm_steel',
        count = config.random_steel_cards,
        isEdition = false
    }, {
        name = 'm_stone',
        count = config.random_stone_cards,
        isEdition = false
    }, {
        name = 'm_gold',
        count = config.random_gold_cards,
        isEdition = false
    }, {
        name = 'm_lucky',
        count = config.random_lucky_cards,
        isEdition = false
    }, {
        name = 'random_edition',
        count = config.random_edition_cards,
        pool = editionAbilities,
        isEdition = true
    }, {
        name = 'random_enhancement',
        count = config.random_enhancement_cards,
        pool = enhancementAbilities,
        isEdition = false
    }}
...
function CardUtils.initializeCustomCardList(deckObj)
    ...
    for k, v in pairs(deck) do
        ...
        if noFaces and (rank == 'K' or rank == 'Q' or rank == 'J') then
            keep = false
        end
        if noNumbered and
            (rank == '2' or rank == '3' or rank == '4' or rank == '5' or rank == '6' or rank == '7' or rank == '8' or
                rank == '9' or rank == 'T') then
            keep = false
        end
    ...
    for k, v in ipairs(card_protos) do
        ...
        local _card = Card(G.deck.T.x, G.deck.T.y, G.CARD_W, G.CARD_H, G.P_CARDS[v.suit .. '_' .. v.rank],
            G.P_CENTERS[v.enhancement or 'c_base'], {
        ...
        if v.seal then
            _card:set_seal(v.seal, true, true)
        end
...
function CardUtils.getCardsFromCustomCardList(deck)
    ...
    for k, v in ipairs(card_protos) do
        local _card = Card(999, 999, G.CARD_W, G.CARD_H, G.P_CARDS[v.suit .. '_' .. v.rank],
            G.P_CENTERS[v.enhancement or 'c_base'])
        _card.uuid = v.key
        if v.edition then
            _card:set_edition({
