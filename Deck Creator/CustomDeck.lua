...
function CustomDeck:fullNew(name, loc_txt, dollars, handSize, discards, hands, reRollCost, jokerSlots, anteScaling,
    consumableSlots, dollarsPerHand, dollarsPerDiscard, jokerRate, tarotRate, planetRate, spectralRate, playingCardRate,
    randomizeRankSuit, noFaces, interestAmount, interestCap, discountPercent, edition, doubleTag, balanceChips,
    editionCount, deckBackIndex, winAnte, inflation, shopSlots, allPolychrome, allHolo, allFoil, allBonus, allMult,
    allWild, allGlass, allSteel, allStone, allGold, allLucky, enableEternalsInShop, boosterAnteScaling, chipsDollarCap,
    discardCost, minus_hand_size_per_X_dollar, allEternal, debuffPlayedCards, flippedCards, uuid, copyDeckConfig,
    customCardList, customCardsSet, customJokerList, customJokersSet, customTarotList, customTarotsSet,
    customPlanetList, customPlanetsSet, customSpectralList, customSpectralsSet, customVoucherList, customVouchersSet,
    broken_glass_money, enhanced_dollars_per_round, negative_joker_money, negative_joker_for_broken_glass,
    balance_percent, double_tag_percent, randomize_ranks, randomize_suits, no_numbered_cards,
    randomize_money_configurable, randomize_money_small, randomize_money_settings, randomize_appearance_rates,
    random_starting_jokers, randomize_hands_discards, random_starting_items, randomly_enable_gameplay_settings,
    one_random_voucher, doubled_probabilities, halved_probabilities, negative_tag_percent, mega_standard_tag_percent,
    full_price_jokers, full_price_consumables, chip_reduction_percent, mult_reduction_percent, draw_to_hand_size,
    chance_to_increase_discard_cards_rank, chance_to_increase_drawn_cards_rank, random_sell_value_increase,
    random_gold_cards, random_polychrome_cards, random_holographic_cards, random_foil_cards, random_edition_cards,
    random_bonus_cards, random_glass_cards, random_lucky_cards, random_steel_cards, random_stone_cards,
...
function CustomDeck.fullNewFromExisting(deck, desc1, desc2, desc3, desc4, updateUUID)
    ...
    return CustomDeck:fullNew(deck.name, {
        name = deck.name,
        text = {
            [1] = desc1 or "",
            [2] = desc2 or "",
    ...
    }, deck.config.dollars, deck.config.hand_size, deck.config.discards, deck.config.hands, deck.config.reroll_cost,
        deck.config.joker_slot, deck.config.ante_scaling, deck.config.consumable_slot, deck.config.extra_hand_bonus,
        deck.config.extra_discard_bonus, deck.config.joker_rate, deck.config.tarot_rate, deck.config.planet_rate,
        deck.config.spectral_rate, deck.config.playing_card_rate, deck.config.randomize_rank_suit,
        deck.config.remove_faces, deck.config.interest_amount, deck.config.interest_cap, deck.config.discount_percent,
        deck.config.edition, deck.config.double_tag, deck.config.balance_chips, deck.config.edition_count,
        deck.config.deck_back_index, deck.config.win_ante, deck.config.inflation, deck.config.shop_slots,
        deck.config.all_polychrome, deck.config.all_holo, deck.config.all_foil, deck.config.all_bonus,
        deck.config.all_mult, deck.config.all_wild, deck.config.all_glass, deck.config.all_steel, deck.config.all_stone,
        deck.config.all_gold, deck.config.all_lucky, deck.config.enable_eternals_in_shop,
        deck.config.booster_ante_scaling, deck.config.chips_dollar_cap, deck.config.discard_cost,
        deck.config.minus_hand_size_per_X_dollar, deck.config.all_eternal, deck.config.debuff_played_cards,
        deck.config.flipped_cards, updateUUID and Utils.uuid() or deck.config.uuid, deck.config.copy_deck_config,
        deck.config.customCardList, deck.config.custom_cards_set, deck.config.customJokerList,
        deck.config.custom_jokers_set, deck.config.customTarotList, deck.config.custom_tarots_set,
        deck.config.customPlanetList, deck.config.custom_planets_set, deck.config.customSpectralList,
        deck.config.custom_spectrals_set, deck.config.customVoucherList, deck.config.custom_vouchers_set,
        deck.config.broken_glass_money, deck.config.enhanced_dollars_per_round, deck.config.negative_joker_money,
        deck.config.negative_joker_for_broken_glass, deck.config.balance_percent, deck.config.double_tag_percent,
        deck.config.randomize_ranks, deck.config.randomize_suits, deck.config.no_numbered_cards,
        deck.config.randomize_money_configurable, deck.config.randomize_money_small,
        deck.config.randomize_money_settings, deck.config.randomize_appearance_rates,
        deck.config.random_starting_jokers, deck.config.randomize_hands_discards, deck.config.random_starting_items,
        deck.config.randomly_enable_gameplay_settings, deck.config.one_random_voucher,
        deck.config.doubled_probabilities, deck.config.halved_probabilities, deck.config.negative_tag_percent,
        deck.config.mega_standard_tag_percent, deck.config.full_price_jokers, deck.config.full_price_consumables,
        deck.config.chip_reduction_percent, deck.config.mult_reduction_percent, deck.config.draw_to_hand_size,
        deck.config.chance_to_increase_discard_cards_rank, deck.config.chance_to_increase_drawn_cards_rank,
        deck.config.random_sell_value_increase, deck.config.random_gold_cards, deck.config.random_polychrome_cards,
        deck.config.random_holographic_cards, deck.config.random_foil_cards, deck.config.random_edition_cards,
        deck.config.random_bonus_cards, deck.config.random_glass_cards, deck.config.random_lucky_cards,
        deck.config.random_steel_cards, deck.config.random_stone_cards, deck.config.random_wild_cards,
        deck.config.random_mult_cards, deck.config.random_enhancement_cards)
end
...
function CustomDeck.getAllDeckBackNames()
    return {"Red", "Blue", "Yellow", "Green", "Black", "Magic", "Nebula", "Ghost", "Abandoned", "Checkered", "Zodiac",
            "Painted", "Anaglyph", "Plasma", "Erratic", "Challenge", "Special", "Fade", "Gold", "Silver", "Glass",
            "Stone", "Lucky", "Wild", "Mult", "Bonus", "White", "Lock", "Soul", "Question", "Question 2", "Gold Seal",
            "Purple Seal", "Red Seal", "Blue Seal", "Random"}
end
